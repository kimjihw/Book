...
import com.example.book.navigation.model.AlarmDTO
import com.example.book.navigation.model.ContentDTO
import com.example.book.navigation.model.FollowDTO
import com.example.book.navigation.util.FcmPush
...
class UserFragment : Fragment() {
	...
	override fun onCreateView(
		...
		fragmentView?.account_recyclerview?.layoutManager = GridLayoutManager(requireActivity(), 2)
	...
	fun requestFollow() {
		...
		firestore?.runTransaction { transaction ->
			...
			if (followDTO.followings.containsKey(uid)) {
				...
				if (fragmentView?.account_btn_follow_signout?.text == "Follow cancel") {
					followDTO?.followingCount = followDTO?.followingCount - 1
					followDTO?.followers?.remove(uid)
				}
			}
